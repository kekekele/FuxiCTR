...
def normalize_feature_type(feature_type):
    ...
    if feature_type in {"int", "integer", "float", "double", "long", "numeric", "number"}:
        return "numeric"
    if feature_type in {"str", "string", "category", "categorical", "id", "enum", "bool", "boolean"}:
        return "categorical"
...
def load_mapping(mapping_csv, key_col, type_col, name_col=None):
    ...
    for _, row in mapping_df.iterrows():
        ...
        feature_name = row[name_col] if name_col and name_col in row and pd.notna(row[name_col]) else "key_{}".format(key)
...
def transform_row(row,
    ...
    ratings = parse_csv_list(row[rating_col], int)
    ...
    if strict and len(ratings) not in (0, periods):
        raise ValueError(
            "user_id={} has {} ratings, expected 0 or {}".format(user_id, len(ratings), periods)
        )
    ...
    for key, period_idx, raw_value in zip(keys, times, values):
        ...
        if strict and pair in seen_pairs:
            raise ValueError("user_id={} has duplicated (key={}, time={})".format(user_id, key, period_idx))
        seen_pairs.add(pair)
...
def build_schema(output_df, key_to_type, key_to_name, periods, user_col, label_col):
    feature_specs = []
    feature_specs.append({"name": user_col, "type": "categorical", "dtype": "str"})

    for period_idx in range(periods):
        feature_specs.append({"name": "rating_t{}".format(period_idx), "type": "numeric", "dtype": "float"})
    ...
    for key in sorted(key_to_type.keys()):
        ...
        for period_idx in range(periods):
            ...
            if column_name in output_df.columns:
                feature_specs.append({"name": column_name, "type": feature_type, "dtype": dtype})
...
def main():
    parser = argparse.ArgumentParser(description="Convert raw NPS CSV data to a DCNv2-friendly wide CSV.")
    parser.add_argument("--input_csv", required=True, help="Raw input CSV path.")
    parser.add_argument("--mapping_csv", required=True, help="Mapping CSV path with key -> feature type.")
    ...
    parser.add_argument("--time_col", default="time", help="Time index list column name.")
    parser.add_argument("--value_col", default="value", help="Sparse value list column name.")
    parser.add_argument("--rating_col", default="rating", help="Rating list column name.")
    parser.add_argument("--label_col", default="label", help="Label column name.")
    parser.add_argument("--mapping_key_col", default="key", help="Mapping CSV key column name.")
    parser.add_argument("--mapping_type_col", default="feature_type", help="Mapping CSV feature type column name.")
    parser.add_argument("--mapping_name_col", default="feature_name",
    ...
    front_columns.extend(["rating_t{}".format(period_idx) for period_idx in range(args.periods)])
    tail_columns = [args.label_col]
    middle_columns = sorted(
        [col for col in output_df.columns if col not in set(front_columns + tail_columns)]
    )
    ...
    if total_unknown_keys:
        print("Warning: ignored unknown keys: {}".format(dict(sorted(total_unknown_keys.items()))))
    if total_invalid_periods:
        print("Warning: ignored invalid (key, time) pairs: {}".format(dict(sorted(total_invalid_periods.items()))))
